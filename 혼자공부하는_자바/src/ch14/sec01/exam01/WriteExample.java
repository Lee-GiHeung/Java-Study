...
public class WriteExample {
	...
	public static void main(String[] args) throws IOException  {
		...
		// 위의 바이트변수를 출력스트림으로 파일을 만듦
		os.write(a);
		os.write(b);
		os.write(c);
		System.out.println("데이터를 파일 출력 스트림에 전송");
		
		// 현재 파일 출력스트림의 버퍼에 보관중인 데이터를 파일로 출력
		os.flush();
		System.out.println("남아 있는 잔여 데이터 파일스트림에 비움");
		
		// 모든 스트림 작업이 끝나면 반드시 닫아줘야함
		// 안하면 메모리에서 제거 되지 않을 수 있음(메모리 누수)
		os.close();
